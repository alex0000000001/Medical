...
namespace Medical.UserControls
{
    public partial class DoctorRating : Form
    {
        ...
        {
            var result = from r in medicalContext.RatingDoctors
                         where r.RatingType.RatingTypeName == "2" &&
                               r.Doctor.DoctorName == this.label_RatingTitle.Text
                         select new
                         {
        ...
        {
            var result = from r in medicalContext.RatingDoctors
                         where r.RatingType.RatingTypeName == "4" && r.Doctor.DoctorName == this.label_RatingTitle.Text
                         select new
                         {
                             所有評分 = r.RatingType.RatingTypeName,
                             評論 = r.Rating
        ...
        {
            var result = from r in medicalContext.RatingDoctors
                         where r.RatingType.RatingTypeName == "4" &&
                               r.Doctor.DoctorName == this.label_RatingTitle.Text
                         select new
                         {
        ...
        {
            var result = from r in medicalContext.RatingDoctors
                         where r.RatingType.RatingTypeName == "5" &&
                               r.Doctor.DoctorName == this.label_RatingTitle.Text
                         select new
                         {
        ...
        private void siticoneButton_three_Click_1(object sender, EventArgs e)
        {
            var result = from r in medicalContext.RatingDoctors
                         where r.RatingType.RatingTypeName == "3" &&
                               r.Doctor.DoctorName == this.label_RatingTitle.Text
                         select new
                         {
