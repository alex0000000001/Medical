...
namespace Medical.UserControls
{
    public partial class DoctorTeam : UserControl
    {
        ...
        {
            textBox_dorctor.Text = "陳";
        }

        private void comboBox_Dorctor_SelectedIndexChanged(object sender, EventArgs e)
        {
            var result = from d in medicalcontext.Doctors.AsEnumerable()
                         where d.DoctorName.Contains(comboBox_Dorctor.Text.ToUpper()) ||
                               d.DoctorName.Contains(comboBox_Dorctor.Text.ToLower())
                         select d;
            this.flowLayoutPanel_ALL_doctor.Controls.Clear();
