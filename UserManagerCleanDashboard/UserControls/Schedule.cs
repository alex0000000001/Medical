...
namespace Medical.UserControls
{
    public partial class Schedule : UserControl
    {
        ...
        public void NewCol(string text)
        {
            DataGridViewButtonColumn column = new DataGridViewButtonColumn();
            column.Name = text;
            column.Text = text;
            column.UseColumnTextForButtonValue = true;
            gvSchedule.Columns.Add(column);
        }

        private void gvSchedule_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            DataGridView dgv = (DataGridView)sender;
            if (dgv.Columns[e.ColumnIndex].Name == "修改")
            {
                DataGridViewButtonCell cell = (DataGridViewButtonCell)dgv[e.ColumnIndex, e.RowIndex];              
                //this.Controls.Clear();
                //this.Controls.Add(new MyDoctor_DoctorWork());
                //this.Controls.Find("MyDoctor_DoctorWork", false)[0].BringToFront();
            }

            if (dgv.Columns[e.ColumnIndex].Name == "刪除")
            {
                DataGridViewButtonCell cell = (DataGridViewButtonCell)dgv[e.ColumnIndex, e.RowIndex];
                //this.Controls.Clear();
                //this.Controls.Add(new MyDoctor_DoctorWork());
